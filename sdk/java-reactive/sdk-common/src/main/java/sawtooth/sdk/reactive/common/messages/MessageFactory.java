...
public class MessageFactory {
	...
	String signerPublicKeyString;
	public byte[] signerPublicKeyEncodedPointByte;
	
	public String getPubliceyString() {
		return signerPublicKeyString;
	}
	
	public void setPublicKeyString(byte[] pubKStr) {
		signerPublicKeyString = FormattingUtils.bytesToHex(pubKStr);
	}
	
	private void setPublicKeyEPoint(byte[] pubKStr) {
	
		
		this.signerPublicKeyEncodedPointByte = pubKStr;
		
	}

	/*public byte[] getPublicKeyByte() {
		return signerPublicKeyByte;
	}
	
	public ByteString getPublicKeyByteString() {
		return signerPublicKeyByteString;
	}*/
	...
	public MessageFactory(String familyName, String digesterAlgo, String familyVersion, ECKey privateKey,
			ECKey publicKey, String... nameSpaces) throws NoSuchAlgorithmException {
		...
		if (privateKey == null) {
			...
			this.signerPrivateKey = SawtoothSigner.generatePrivateKey(new SecureRandom(
		...
		if (publicKey == null) {
			setPublicKeyString(signerPrivateKey.getPubKey());
			setPublicKeyEPoint(signerPrivateKey.getPubKeyPoint().getEncoded(true));
		}
		else {
			setPublicKeyString(publicKey.getPubKey());
			setPublicKeyEPoint(publicKey.getPubKeyPoint().getEncoded(true));
		}

		List<String> binNameSpaces = new ArrayList<String>();
		for (String eachNS : nameSpaces) {
			binNameSpaces.add(MESSAGEDIGESTER_512.get().digest(eachNS.getBytes(StandardCharsets.US_ASCII)).toString()
					.substring(0, 6));
		}
	...
	public TransactionHeader createTransactionHeader(String payloadSha512, List<String> inputs, List<String> outputs,
	...
	private TpRegisterRequest createTpRegisterRequest() {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug(String.format("Register request : Family %s, Namespaces %s, Version %s", familyName,
					Arrays.deepToString(nameSpaces), familyVersion));
		}
	...
	public TpProcessRequest createTpProcessRequest(String contextId, StringBuffer payload, List<String> inputs,
		...
		reqBuilder.setContextId(contextId)
			.setHeader(createTransactionHeader(hexFormattedDigest, inputs, outputs, dependencies, Boolean.TRUE, batcherPubKey));
		
				
		reqBuilder.setPayload(ByteString.copyFrom(hexFormattedDigest,StandardCharsets.ISO_8859_1));
		/*
		 * reqBuilder.setPayload(
		 * ByteString.copyFrom(MESSAGEDIGESTER.digest(payload.toString().getBytes(
	...
	private Transaction createTransaction(StringBuffer payload, List<String> inputs, List<String> outputs,
		...
		transactionBuilder.setPayload(ByteString.copyFrom(payload.toString(),StandardCharsets.US_ASCII));
		TransactionHeader header = createTransactionHeader(generateHASH512Hex(payload), inputs, outputs, dependencies, Boolean.TRUE,
				batcherPubKey);
	...
	private Transaction createTransactionFromProcessRequest(Message processRequest) throws InvalidProtocolBufferException, NoSuchAlgorithmException{
		Transaction.Builder transactionBuilder = Transaction.newBuilder();
		...
		TransactionHeader header = createTransactionHeader(hexFormattedDigest, theRequest.getHeader().getInputsList(), theRequest.getHeader().getOutputsList(), theRequest.getHeader().getDependenciesList(), Boolean.TRUE,
	...
	public Batch createBatch(List<? extends Message> transactions, boolean trace) {
		...
		List<String> txnSignatures = transactions.stream().map(et -> {
			...
			try {
				if (et.getMessageType().equals(MessageType.TP_PROCESS_REQUEST)) {
					...
					Transaction toAdd = createTransactionFromProcessRequest(et);
					...
					transactionListBuilder.addTransactions(theTransaction);
		...
		String headerSignature = SawtoothSigner.signHexSequence(signerPrivateKey, batchHeader.toByteArray()); 
		
		Batch.Builder batchBuilder = Batch.newBuilder();
		batchBuilder.setHeader(batchHeader.toByteString());
		batchBuilder.setHeaderSignature(headerSignature);
		batchBuilder.addAllTransactions(transactionListBuilder.build().getTransactionsList()).setTrace(trace);
	...
	/**
	 * generate a random String using the sha-512 algorithm, to correlate sent messages.
	 * with futures
	 * 
